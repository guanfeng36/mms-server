...
func maxMethod(host_id string, cycle int, trigger *types.Trigger) (*types.TriggerResultSet, error) {
	...
	params := NewQueryParams(host_id, cycle, 0, trigger.Tags, "sum", trigger.Metric, trigger.Dsname, trigger.Function, trigger.Orders)
	results, err := tsdbClient.Query(params, trigger.Metric)
	if err != nil {
		lg.Error(err.Error())
		return nil, err
	}
...
func minMethod(host_id string, cycle int, trigger *types.Trigger) (*types.TriggerResultSet, error) {
	...
	query_str := NewQueryParams(host_id, cycle, 0, trigger.Tags, "sum", trigger.Metric, trigger.Dsname, trigger.Function, trigger.Orders)
	results, err := tsdbClient.Query(query_str, trigger.Metric)
	if err != nil {
		lg.Error(err.Error())
		return nil, err
	}
...
func ratioMethod(host_id string, cycle int, trigger *types.Trigger) (*types.TriggerResultSet, error) {
	...
	params = NewQueryParams(host_id, start_time, end_time, trigger.Tags, "sum", trigger.Metric, trigger.Dsname, trigger.Function, trigger.Orders)
	results_ago, err := tsdbClient.Query(params, trigger.Metric)
	if err != nil {
		lg.Error(err.Error())
		return nil, err
	}
...
func lastMethod(host_id string, cycle int, trigger *types.Trigger) (*types.TriggerResultSet, error) {
	...
	for _, result := range results {
		...
		for index, value_time := range value_times {
			values[index] = result.Dps[value_time]
		}
...
func unit(cycle, number int) (start_time, end_time int) {
	start_time = 24*60 + cycle
	end_time = 24 * 60
