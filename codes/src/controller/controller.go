...
func (this *Controller) doAlarmExecute(strategy_event *StrategyEvent) {
	executes := mydb.GetExecutes(strategy_event.StrategyID)
	for _, execute := range executes {
		var err error
		var msg AlarmExecuteMsg
		msg.Identify = ""
		msg.MsgBody.Type = AMQP_MSG_TYPE_ALARM_EXECUTE

		mydb.DelExecuteResult(strategy_event.ID)
		if err != nil {
			lg.Error("Failed to delete old execute result by strategy event id %v.", strategy_event.ID)
			return
		}

		msg.MsgBody.Task_id, err = mydb.CreateExecuteResult(strategy_event, execute)
		if err != nil {
			lg.Error("Failed to create execute result by strategy event id %v.", strategy_event.ID)
			return
		}

		msg_json, err1 := json.Marshal(msg)
		if err1 != nil {
			lg.Error("Failed to encode amqp msg to json.")
			return
		}

		// err = AmqpSend(GlobalConfig.AMQP_URL, GlobalConfig.AMQP_EXCHANGE, GlobalConfig.AMQP_QUEUE_TASK_MANAGE, []byte(msg_json))
		// if err != nil {
		// 	lg.Error("Failed to send msg to amqp,url:%v,exchange:%v,queue:%v", GlobalConfig.AMQP_URL, GlobalConfig.AMQP_EXCHANGE, GlobalConfig.AMQP_QUEUE_TASK_MANAGE)
		// 	return
		// }
		...
		msg_to_send.Topic = GlobalConfig.KAFKA_TASK_MGT_TOPIC
		msg_to_send.Partition = int32(-1)
		msg_to_send.Value = sarama.ByteEncoder(msg_json)
		...
		partition, offset, err := producer.SendMessage(msg_to_send)

		if err != nil {
			lg.Error("Failed to produce message: ", err)
			return
		}
		lg.Debug("partition=%d, offset=%d\n", partition, offset)
	}
}
...
func (this *Controller) processResult(strategy_result *StrategyResult) {
	...
	if new_strategy_event != nil {
		...
		if new_strategy_event.Count < task.Strategy.AlarmCount || task.Strategy.AlarmCount == 0 {
			...
			this.doAlarmExecute(strategy_event)
		}
		mydb.UpdateStrategyEvent(new_strategy_event, trigger_event_sets)
		return
	}
