...
func (this *db) GetHostsByStrategyID(strategy_id int) []*Host {
	rows, err := this.Query("SELECT `id`, `ipv4`, `mac_addr` FROM postinfo WHERE id IN (SELECT host_id FROM strategy_host WHERE strategy_id=?)", strategy_id)
	if err != nil {
		lg.Error(err.Error())
		return nil
	}
	...
	for rows.Next() {
		host := Host{}
		if err := rows.Scan(&host.ID, &host.IP, &host.Hostname); err != nil {
			lg.Error(err.Error())
			continue
		}
		host.Status = "1"
...
func (this *db) GetGroupsByStrategyID(strategy_id int) []*Group {
	rows, err := this.Query("SELECT `group_id`, `group_name` FROM `device_group` WHERE `group_id` IN (SELECT `group_id` FROM `strategy_group` WHERE `strategy_id`=?)", strategy_id)
	if err != nil {
		lg.Error(err.Error())
		return nil
	}
...
func (this *db) GetHostsByGroupID(group_id int) []*Host {
	rows, err := this.Query("SELECT `id`, `ipv4`, `mac_addr` FROM postinfo WHERE id IN (SELECT postinfo_id FROM device_group_devices WHERE device_group_id = ?)", group_id)
	if err != nil {
		lg.Error(err.Error())
		return nil
	}
...
func (this *db) GetTriggersByStrategyID(strategy_id int) map[string]*Trigger {
	rows, err := this.Query("SELECT `id`, `strategy_id`, `metric`, `dsname`, `function`, `orders`, `tags`, `number`, `index`, `name`, `method`, `symbol`, `threshold`, `description` FROM `trigger` WHERE `strategy_id` = ?", strategy_id)
	if err != nil {
		lg.Error(err.Error())
		return nil
	}
	...
	for rows.Next() {
		trigger := Trigger{}
		if err := rows.Scan(&trigger.ID, &trigger.StrategyID, &trigger.Metric, &trigger.Dsname, &trigger.Function, &trigger.Orders, &trigger.Tags, &trigger.Number, &trigger.Index, &trigger.Name, &trigger.Method, &trigger.Symbol, &trigger.Threshold, &trigger.Description); err != nil {
			lg.Error(err.Error())
			continue
		}
...
func (this *db) GetExecutes(strategy_id int) []*Execute {
	rows, err := this.Query("SELECT `id`, `strategy_id`, `type`, `command` FROM `execute` WHERE `strategy_id` = ?", strategy_id)
	if err != nil {
		lg.Error(err.Error())
		return nil
	}
	...
	for rows.Next() {
		execute := Execute{}
		if err := rows.Scan(&execute.ID, &execute.StrategyID, &execute.Type, &execute.Command); err != nil {
			lg.Error(err.Error())
			continue
		}
		executes = append(executes, &execute)
	}
	return executes
}

func (this *db) DelExecuteResult(strategy_event_id int64) error {
	stmt_delete, err := this.Prepare("DELETE FROM `execute_result` WHERE `strategy_event_id` = ?")
	_, err = stmt_delete.Exec(strategy_event_id)
	if err != nil {
		lg.Error(err.Error())
		return err
	}
...
func (this *db) CreateExecuteResult(strategy_event *StrategyEvent, execute *Execute) (int64, error) {
	stmt, err := this.Prepare("INSERT INTO `execute_result` VALUES (0, ?, ?, ?, ?, ?, 0)")
	if err != nil {
		lg.Error(err.Error())
		return -1, err
	}
...
func (this *db) CreateStrategyEvent(strategy_event *StrategyEvent, trigger_event_sets map[string][]*TriggerEvent) (int64, error) {
	...
	for _, trigger_event_set := range trigger_event_sets {
		for _, trigger_event := range trigger_event_set {
			stmt_insert, err := tx.Prepare("INSERT INTO `trigger_event` VALUES (0, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")
			if err != nil {
				lg.Error(err.Error())
				return -1, err
			}
...
func (this *db) UpdateStrategyEvent(strategy_event *StrategyEvent, trigger_event_sets map[string][]*TriggerEvent) error {
	...
	stmt, err := tx.Prepare("UPDATE `strategy_event` SET `update_time` = ?, `alarm_count` = ?,`count` = ?, `status` = ?, `process_user` = ?, `process_comments` = ?, `process_time` = ? WHERE `id` = ?")
	if err != nil {
		lg.Error(err.Error())
		return err
	}
...
func (this *db) GetStrategyEvent(strategy_id, status int, host_id string) *StrategyEvent {
	...
	var strategy_alarm_count int;
        if err2 := this.QueryRow("SELECT `alarm_count` FROM `strategy` WHERE `id` = ? ",
                strategy_id).Scan(&strategy_alarm_count); err2 != nil {
        ...
        if strategy_alarm_count < strategy_event.Count {
                strategy_event.AlarmCount = strategy_event.Count
        }
        else{
        	strategy_event.AlarmCount = strategy_alarm_count
        }
...
func (this *db) GetUsersByGroups(action_id int) []*User {
	sql := fmt.Sprintf("SELECT `id`, `username`, `email` FROM `auth_user` WHERE `id` IN (SELECT `user_id` FROM `auth_user_groups` WHERE `group_id` IN (SELECT `user_group_id` FROM action_user_group WHERE action_id = %d))", action_id)
...
func (this *db) GetUsers(action_id int) []*User {
	sql := fmt.Sprintf("SELECT `id`, `username`, `email` FROM `auth_user` WHERE `id` IN (SELECT `user_id` FROM action_user WHERE action_id = %d)", action_id)
	rows, err := this.Query(sql)
	if err != nil {
		lg.Error(err.Error())
		return nil
